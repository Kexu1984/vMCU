...
static uint8_t StartProgramTask(void *arg, uint32_t flag)
{
    ...
    if (STATUS_SUCCESS != ret)
    {
        DEBUGMSG(DEBUG_ZONE_ERROR, ("[FLASH][Err]Program task TEST_FAIL in address: \
                                    0x%x, state: 0x%x\r\n", g_flashAddr, EFLASH->CSR));
    }
    else
    {
...
void FLASH_WriteProtectTest(void *param, int argc, const char** argv)
{
    ...
    Debug_Printf("[EFLASH]:TestWriteProtect:index:0x%x,protectValue[0]:0x%x,protectValue[1]:0x%x\r\n", index, protectValue[0], protectValue[1]);
...
int main(int argc, const char **argv)
{
    ...
    result = register_device(7, EFLASH_BASE, 0x10000);
    result |= register_device(7, PFLASH_BASE_ADDRESS, PFLASH_SIZE);
    result |= register_device(7, DFLASH_BASE_ADDRESS, DFLASH_SIZE);
    result |= register_device(7, CINFO_BASE, CINFO_END - CINFO_BASE);
    result |= register_device(7, DINFO_BASE, DINFO_END - DINFO_BASE);
    if (result != 0) {
